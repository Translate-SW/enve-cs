...
CanvasSettingsWidget::CanvasSettingsWidget(QWidget* const parent) :
    ...
    pathColorsLay->addPair(new QLabel("Velikost uzlu"), nodeLayout);
    ...
    pathColorsLay->addPair(new QLabel("Rozpuštěný režim"), dissolvedNodeLayout);
    ...
    pathColorsLay->addPair(new QLabel("Velikost pro kontrolu"), controlLayout);

    addLayout(pathColorsLay);

    addSeparator();

    mRtlSupport = new QCheckBox("Podpora jazyků RTL", this);
