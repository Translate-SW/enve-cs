...
void MainWindow::setupMenuBar() {
    ...
    mFileMenu = mMenuBar->addMenu(tr("Soubor", "MenuBar"));
    mFileMenu->addAction(tr("Nový...", "MenuBar_File"),
                         this, &MainWindow::newFile,
                         Qt::CTRL + Qt::Key_N);
    mFileMenu->addAction(tr("Otevřít...", "MenuBar_File"),
                         this, qOverload<>(&MainWindow::openFile),
                         Qt::CTRL + Qt::Key_O);
    mRecentMenu = mFileMenu->addMenu(tr("Nedávno otevřené", "MenuBar_File"));
    mFileMenu->addSeparator();
    mFileMenu->addAction("Link...",
                         this, &MainWindow::linkFile,
                         Qt::CTRL + Qt::Key_L);
    mFileMenu->addAction(tr("Importovat soubor...", "MenuBar_File"),
                         this, qOverload<>(&MainWindow::importFile),
                         Qt::CTRL + Qt::Key_I);
    mFileMenu->addAction(tr("Importovat sekvenci obrázků", "MenuBar_File"),
                         this, &MainWindow::importImageSequence);
    mFileMenu->addSeparator();
    mFileMenu->addAction(tr("Revert", "MenuBar_File"),
                         this, &MainWindow::revert);
    mFileMenu->addSeparator();
    mFileMenu->addAction(tr("Uložit", "MenuBar_File"),
                         this, qOverload<>(&MainWindow::saveFile),
                         Qt::CTRL + Qt::Key_S);
    mFileMenu->addAction(tr("Uložit jako...", "MenuBar_File"),
                         this, [this]() { saveFileAs(); },
                         Qt::CTRL + Qt::SHIFT + Qt::Key_S);
    mFileMenu->addAction(tr("Uložit jako zálohu", "MenuBar_File"),
                         this, &MainWindow::saveBackup);

    const auto exportMenu = mFileMenu->addMenu(tr("Export", "MenuBar_File"));
    exportMenu->addAction(tr("Exportovat SVG soubor...", "MenuBar_File"),
                          this, &MainWindow::exportSVG);
    mFileMenu->addSeparator();
    mFileMenu->addAction(tr("Zavřít oknp", "MenuBar_File"),
                         this, &MainWindow::closeProject);
    mFileMenu->addSeparator();
    mFileMenu->addAction(tr("Opustit", "MenuBar_File"),
                         this, &MainWindow::close);

    mEditMenu = mMenuBar->addMenu(tr("Upravit", "MenuBar"));

    const auto undoQAct = mEditMenu->addAction(tr("Zpět", "MenuBar_Edit"));
    undoQAct->setShortcut(Qt::CTRL + Qt::Key_Z);
    mActions.undoAction->connect(undoQAct);

    const auto redoQAct = mEditMenu->addAction(tr("Znovu", "MenuBar_Edit"));
    ...
    {
        const auto qAct = new NoShortcutAction(tr("Kopírovat", "MenuBar_Edit"));
    ...
    {
        const auto qAct = new NoShortcutAction(tr("Vyjmout", "MenuBar_Edit"));
        mEditMenu->addAction(qAct);
    ...
    {
        const auto qAct = new NoShortcutAction(tr("Vložit", "MenuBar_Edit"));
        mEditMenu->addAction(qAct);
    ...
    {
        const auto qAct = new NoShortcutAction(tr("Duplikovat", "MenuBar_Edit"));
        mEditMenu->addAction(qAct);
    ...
    {
        const auto qAct = new NoShortcutAction(tr("Smazat", "MenuBar_Edit"));
        mEditMenu->addAction(qAct);
    ...
    mEditMenu->addAction(new NoShortcutAction(
                             tr("Vybrat vše", "MenuBar_Edit"),
                             &mActions, &Actions::selectAllAction,
                             Qt::Key_A, mEditMenu));
    mEditMenu->addAction(new NoShortcutAction(
                             tr("Invertovat sekvenci", "MenuBar_Edit"),
                             &mActions, &Actions::invertSelectionAction,
                             Qt::Key_I, mEditMenu));
    mEditMenu->addAction(new NoShortcutAction(
                             tr("Vyčistit sekvenci", "MenuBar_Edit"),
                             &mActions, &Actions::clearSelectionAction,
                             Qt::ALT + Qt::Key_A, mEditMenu));
    mEditMenu->addSeparator();
    mEditMenu->addAction(tr("Nastavit...", "MenuBar_Edit"), [this]() {
...
//    mSelectSameMenu->addAction("Object Type");
...

    {
        const auto qAct = mObjectMenu->addAction(
                    tr("Otočit o 90° doprava", "MenuBar_Object"));
        mActions.rotate90CWAction->connect(qAct);
    }

    {
        const auto qAct = mObjectMenu->addAction(
                    tr("Otořit o 90° doleva", "MenuBar_Object"));
        mActions.rotate90CCWAction->connect(qAct);
    }

    {
        const auto qAct = mObjectMenu->addAction(
                    tr("Horizontální převrácení", "MenuBar_Object"));
    ...
    {
        const auto qAct = mObjectMenu->addAction(
                    tr("Veftikální převrácení", "MenuBar_Object"));
    ...
    const auto transformMenu = mObjectMenu->addMenu(
                tr("Transformovat", "MenuBar_Object"));
    ...
    mPathMenu = mMenuBar->addMenu(tr("Umíštění", "MenuBar"));

    const auto otpQAct = mPathMenu->addAction(
                tr("Vložit objekt do umíštění...", "MenuBar_Path"));
    mActions.objectsToPathAction->connect(otpQAct);

    const auto stpQAct = mPathMenu->addAction(
                tr("Odstranit z umíštění", "MenuBar_Path"));
...
//    mEffectsMenu->addAction("Blur");
    ...
    mSceneMenu = mMenuBar->addMenu(tr("Scéna", "MenuBar"));

    mSceneMenu->addAction(tr("Nová scéna", "MenuBar_Scene"), this, [this]() {
        SceneSettingsDialog::sNewSceneDialog(mDocument, this);
    });

    {
        const auto qAct = mSceneMenu->addAction(
                    tr("Odstranit scénu", "MenuBar_Scene"));
        mActions.deleteSceneAction->connect(qAct);
    }

    mSceneMenu->addSeparator();

    mSceneMenu->addAction(tr("Přidat do renderování", "MenuBar_Scene"),
    ...
    {
        const auto qAct = mSceneMenu->addAction(
                    tr("Nastavit...", "MenuBar_Scene"));
        mActions.sceneSettingsAction->connect(qAct);
    }

    mViewMenu = mMenuBar->addMenu(tr("Zobrazit", "MenuBar"));

    const auto filteringMenu = mViewMenu->addMenu(
                tr("Filtrovat", "MenuBar_View"));

    mNoneQuality = filteringMenu->addAction(
                tr("Žádný", "MenuBar_View_Filtering"), [this]() {
    ...
    mLowQuality = filteringMenu->addAction(
                tr("Nízký", "MenuBar_View_Filtering"), [this]() {
    ...
    mMediumQuality = filteringMenu->addAction(
                tr("Střední", "MenuBar_View_Filtering"), [this]() {
    ...
    mHighQuality = filteringMenu->addAction(
                tr("Vysoký", "MenuBar_View_Filtering"), [this]() {
    ...
    mDynamicQuality = filteringMenu->addAction(
                tr("Dynamický", "MenuBar_View_Filtering"), [this]() {
    ...
    mRasterEffectsVisible = mViewMenu->addAction(
                tr("Rasterové efekty", "MenuBar_View"));
    ...
    mPathEffectsVisible = mViewMenu->addAction(
                tr("efekty umíštění", "MenuBar_View"));
    ...
    mPanelsMenu = mViewMenu->addMenu(tr("Docky", "MenuBar_View"));

    mSelectedObjectDockAct = mPanelsMenu->addAction(
                tr("Zvolené objekty", "MenuBar_View_Docks"));
    ...
    mTimelineDockAct = mPanelsMenu->addAction(
                tr("Časová osa", "MenuBar_View_Docks"));
    ...
    mFillAndStrokeDockAct = mPanelsMenu->addAction(
                tr("Zaříznout a oddělit", "MenuBar_View_Docks"));
    ...
    const auto help = mMenuBar->addMenu(tr("Nápověda", "MenuBar"));

    help->addAction(tr("Licence", "MenuBar_Help"), this, [this]() {
    ...
    mMenuBar->addAction(tr("Podpořte enve!", "MenuBar"), this, []() {
