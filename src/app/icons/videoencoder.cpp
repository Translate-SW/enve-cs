...
static AVFrame *allocPicture(enum AVPixelFormat pix_fmt,
    ...
    if(!picture) RuntimeThrow("Nebyl přidělený Frame.");
    ...
    if(ret < 0) AV_RuntimeThrow(ret, "Nepodařilo se načíst Framová data.")
...
static void openVideo(const AVCodec * const codec, OutputStream * const ost) {
    ...
    if(ret < 0) AV_RuntimeThrow(ret, "Nepodařilo se otevřít kodek")

    /* Allocate the encoded raw picture. */
    ost->fDstFrame = allocPicture(c->pix_fmt, c->width, c->height);
    if(!ost->fDstFrame) RuntimeThrow("Nepodařilo se načíst obrázek");

    /* copy the stream parameters to the muxer */
    ret = avcodec_parameters_from_context(ost->fStream->codecpar, c);
    if(ret < 0) AV_RuntimeThrow(ret, "Nepodařilo se zkpírovat parametry streamu.")
}
...
//    }
    ...
    if(!ost->fStream) RuntimeThrow("Nepodařilo se načíst vzorek streamu.");

    AVCodecContext * const c = avcodec_alloc_context3(codec);
    if(!c) RuntimeThrow("Nepodařilo se načíst encoding context");
...
//        return nullptr;
    ...
    if(!ost->fSwsCtx) {
        ...
        if(!ost->fSwsCtx)
            RuntimeThrow("Nepodařilo se inicializovat konverzní kontext");
    }
    SkPixmap pixmap;
    ...
    if(ret < 0) AV_RuntimeThrow(ret, "Není možné zapisovat do AVFrame")
...
static void writeVideoFrame(AVFormatContext * const oc,
    ...
    try {
        ...
        RuntimeThrow("Nepodařilo se načíst frame videsa"
    }


    // encode the image
    const int ret = avcodec_send_frame(c, frame);
    if(ret < 0) AV_RuntimeThrow(ret, "Nastala chyba při dokončování enkódování"
    ...
    while(ret >= 0) {
        ...
        if(recRet >= 0) {
            ...
            if(interRet < 0) AV_RuntimeThrow(interRet, "Nastala   me")
        } else if(recRet == AVERROR(EAGAIN) || recRet == AVERROR_EOF) {
            *encodeVideo = ret != AVERROR_EOF;
            break;
        } else {
            AV_RuntimeThrow(recRet, "Nastala chyba při zapisování videa.")
        }
...
//    }
    ...
    ost->fStream = avformat_new_stream(oc, nullptr);
    if(!ost->fStream) RuntimeThrow("Nepodařilo se aktivovat zanamenávání.");

    AVCodecContext * const c = avcodec_alloc_context3(codec);
    if(!c) RuntimeThrow("Nepodařilo se načíst kontext enkódování");
    ost->fCodec = c;
    ...
    if(!ost->fSwrCtx) RuntimeThrow("Nastala chyba při převzorkování kontextu.");
    ...
    if(!swr_is_initialized(ost->fSwrCtx)) {
        RuntimeThrow("Video nebylo možné převzorkovat.");
    }
...
static AVFrame *allocAudioFrame(enum AVSampleFormat sample_fmt,
    ...
    if(!frame) RuntimeThrow("Nepodařilo se načíst frame audia");
    ...
    if(nb_samples) {
        ...
        if(ret < 0) AV_RuntimeThrow(ret, "Chyba při načítání an audio nárazníku")
    }
...
static void openAudio(const AVCodec * const codec, OutputStream * const ost,
    ...
    if(!ost->fDstFrame) RuntimeThrow("Nepodařilo se alokovat frame audia");

    ost->fSrcFrame = allocAudioFrame(inSound.fSampleFormat,
                                     inSound.fChannelLayout,
                                     inSound.fSampleRate,
                                     nb_samples);
    if(!ost->fSrcFrame) RuntimeThrow("Nepodařilo se aktivovat dočasný audio výstup");

    /* copy the stream parameters to the muxer */
    const int parRet = avcodec_parameters_from_context(ost->fStream->codecpar, c);
    if(parRet < 0) AV_RuntimeThrow(parRet, "Nepodařilo se zkopírovat parametry streamu")
}
...
static void encodeAudioFrame(AVFormatContext * const oc,
    ...
    if(ret < 0) AV_RuntimeThrow(ret, "Chyba při dokončování enkódování framů")
    ...
    while(true) {
        ...
        if(recRet >= 0) {
            ...
            /* Write the compressed frame to the media file. */
            const int interRet = av_interleaved_write_frame(oc, &pkt);
            if(interRet < 0) AV_RuntimeThrow(interRet, "Chyba při zapisování audio framů")
        } else if(recRet == AVERROR(EAGAIN) || recRet == AVERROR_EOF) {
            *encodeAudio = recRet == AVERROR(EAGAIN);
            break;
        } else {
            AV_RuntimeThrow(recRet, "Chyba při enkódování framů audia")
        }
    }
}
...
//    ost->fNextPts += ost->fDstFrame->nb_samples;
    ...
    try {
        ...
        RuntimeThrow("Chyba při enkódování framů audia");
    }
...
void VideoEncoder::startEncodingNow() {
    ...
    if(!mOutputFormat) {
        ...
        if(!mOutputFormat) {
            RuntimeThrow("Není poskytnut AVOutputFormat. "
    ...
    if(mOutputSettings.fVideoCodec && mOutputSettings.fVideoEnabled) {
        try {
            ...
            RuntimeThrow("Chyba při přidávání video   stopy");
        }
        mEncodeVideo = true;
    }
    ...
    if(mOutputFormat->audio_codec != AV_CODEC_ID_NONE &&
       mOutputSettings.fAudioEnabled && soundComp->hasAnySounds()) {
        ...
        try {
            ...
            RuntimeThrow("Chyba při přidávání audio stopy");
        }
        mEncodeAudio = true;
    }
    if(!mEncodeAudio && !mEncodeVideo) RuntimeThrow("Nejsouk dispozici stopy k renderování.");
    // open streams
    if(mEncodeVideo) {
        try {
...
void VideoEncoder::interrupEncoding() {
    ...
    mRenderInstanceSettings->setCurrentState(RenderState::none, "Interpretováno");
    finishEncodingNow();
    ...
    while((mEncodeVideo && hasVideo) || (mEncodeAudio && hasAudio)) {
        ...
        if(encodeVideo) {
            ...
            try {
                ...
                RuntimeThrow("Chyba při zapisování framů videa");
            }
        ...
        if(encodeAudio) {
            try {
                ...
                RuntimeThrow("Chyba při zavádění audio stopy");
            }
...
void VideoEncoder::afterProcessing() {
    ...
    if(mInterruptEncoding) {
        ...
        mRenderInstanceSettings->setCurrentState(RenderState::error, "Chyba");
        finishEncodingNow();
